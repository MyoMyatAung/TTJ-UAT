...
import { check_answer_forgot } from "../../../services/userService";
...
const SecQues: React.FC<SecQuesProps> = ({ ques, ques_id, email,graphicKey }) => {
  ...
  const validateAns = (username: string) => {
    const lengthValid = ans.length >= 0;
    return lengthValid;
  };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const { data } = await check_answer_forgot(formData);
      if (data.session_token) {
        const tokenData = data.session_token;
        setSession_token(tokenData);
        setIsVisible(false);
        setShowReset(true);
      }
    } catch (error: any) {
